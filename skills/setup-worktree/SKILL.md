...
I've checked out branch `[BRANCHNAME]` for you. To start implementation, create a new session with the following command:
...
To start implementation, create a new session with the following command:
...
1. Figure out what task you're working on - you should have the slug to .artifacts/TASKSLUG already

1a. once you have a task dir, read ticket.md and outline.md if present:

```
Read(.artifacts/TASKSLUG/ticket.md)
Read(.artifacts/TASKSLUG/...outline...md)
```
...
```text
use the rpi:implement-outline skill for task .artifacts/{{ TASKSLUG }}
```
