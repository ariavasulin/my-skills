...
3. After the user answers:
   - **Build out the Document** based on the answer - add/enhance System Design or Program Design (or both), don't just log the answer
...
1. **Find and read the task directory**:
   ...
   - **IMPORTANT**: DO NOT use Glob or Grep on .artifacts — it may be a symlink
...
7. **When the user says they're done or the design is fully fleshed out**, read the final output template:
   ...
   - Respond following the selected template exactly. Do not include a summary or other information.
...
- Write HTML files to `.artifacts/{task-slug}/diagram-{description}.html`
