...
- `docPath`: Path to the existing research questions document (e.g., `.artifacts/ENG-XXXX-description/NN-research-questions-DESCRIPTION.md`)
- Optionally, a ticket file path containing comments with feedback
- If only a directory is provided, check the task's artifact directory for a research questions document: `ls -La .artifacts/<task slug>` from your prompt. Do NOT use grep or glob or run ls without `-L` or with `-l`, as the directory may be a symlink.
