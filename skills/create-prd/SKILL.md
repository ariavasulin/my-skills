...
<instructions>
Write the doc to `.artifacts/ENG-XXXX-description/NN-prd-DESCRIPTION.md` (next zero-padded `NN-` index in the task dir; `DESCRIPTION` is a 2-4 word kebab-case slug).
...
- Write HTML files to `.artifacts/{task-slug}/mockup-{description}.html`
...
Display inline:
```task-artifact
.artifacts/{task-slug}/mockup-{description}.html
```
...
<instructions>
When the solution is approved, read the final answer template and follow it exactly (it points to the next step - the TDD): `Read({SKILLBASE}/references/prd_final_answer_resolved.md)`.
</instructions>
