...
- If only a directory is provided, or a directory is not provided, check the task's artifact directory from your prompt: `ls -La .artifacts/<task slug>`. Do NOT use grep or glob or `ls` or `ls -l`, as the directory may be a symlink.
...
2. **Check for related task content**:
   - If a path in `.artifacts/TASKNAME` is mentioned, use `ls -La .artifacts/TASKNAME`
...
2. **Write the structure outline** to `.artifacts/ENG-XXXX-description/NN-structure-outline-DESCRIPTION.md`
   - First, find the task directory: `ls -La .artifacts | grep -i "eng-XXXX"`
   ...
   - Directory naming:
     - With ticket: `.artifacts/ENG-1478-parent-child-tracking/04-structure-outline-parent-child-tracking.md`
     - Without ticket: `.artifacts/improve-error-handling/04-structure-outline-error-handling.md`
