...
Spark accounts are read-only, so `spark draft` is unavailable. Use Spark for everything leading up to the send — finding threads, looking up contacts, searching for context — then hand off to **Composio's Gmail CLI** for drafting and sending.

**IMPORTANT: Always use the `composio execute` CLI commands below via the Bash tool. Do NOT use the Gmail MCP tool (`mcp__claude_ai_Gmail__*`) — it requires separate OAuth and is not the right integration here. Composio is already authenticated and ready to go.**

**Create a draft** (appears in Gmail Drafts for review before sending):

```bash
composio execute GMAIL_CREATE_EMAIL_DRAFT -d '{
...
Both support `cc`, `bcc`, and multiple recipients:
...
**Default to creating a draft** so the user can review it in Gmail before sending. Only use `GMAIL_SEND_EMAIL` when the user explicitly asks to send immediately.

Use `spark contacts` to look up recipient addresses and `spark search` or `spark thread` to gather context (original subject lines, prior conversation) before composing.
...
2. `spark thread <ID>` - read the conversation for context and grab the subject line
3. `composio execute GMAIL_CREATE_EMAIL_DRAFT -d '{ "recipient_email": "sender@example.com", "subject": "Re: Original Subject", "body": "Thanks for the update!" }'`
4. Tell the user the draft is ready for review in Gmail. If they say to send it directly, use `GMAIL_SEND_EMAIL` instead.
