...
2. **Check for related task content**:
   - If a path in `.artifacts/TASKNAME` is mentioned, use `ls -La .artifacts/TASKNAME`. Do NOT use `ls ` or `ls -l` as the directory may be a symlink.
...
7. **Update the user**
   ...
   - Respond following the template exactly. Do not include a summary or other information.
