---
name: implement-plan
description: phased implementation of a structured plan you must use this skill when asked to implement a plan file in .artifacts/*
...
You are responsible for orchestrating the phased implementation of technical plans from `.artifacts/`. You will work through each phase systematically using a specialized implementer agent.
...
If you were provided with a task artifact directory like `.artifacts/<task slug>` you should list the contents with `ls -La` to locate the plan file inside of it, e.g. `ls -La .artifacts/<task slug>`. Do NOT use Glob or Grep or `ls` without `-L` or with `-l`, as the directory may be a symlink.
...
Example:
```
Implement Phase [N] of the plan at .artifacts/ENG-XXXX-description/NN-plan-DESCRIPTION.md
...
- remember - the .artifacts/ directory should not be committed, its a symlink to another repo
